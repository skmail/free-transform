  ...
  for (let item of snappedResults) {
    ...
    if (matchedToPoint_[0] === to[0] || matchedToPoint_[1] === to[1]) {
      results.push({
        ...item,
        from: [
          matchedToPoint[0] + position[0],
          matchedToPoint[1] + position[1],
        ],
      });
    }
  }
...
const getSurroundings = (
  ...
  let minX = Infinity;
  let minY = Infinity;
  for(const _origin of origin){
    for (const point of points) {
      if (point.x !== undefined && inRange(_origin[0], point.x)) {
        const distance = point.x - _origin[0];
      ...
      if (point.y !== undefined && inRange(_origin[1], point.y)) {
        const distance = point.y - _origin[1];
